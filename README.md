# LP — Diagnóstico de Código gerado por IA · campanha ChatGPT (Lughy)
...
## Relação com a LP Diagnóstico IA

Esta é uma **cópia autônoma** de `LP Diagnostico IA`, criada para rodar a campanha do
ChatGPT em separado. Projeto próprio, repositório próprio, deploy próprio: nada aqui
depende do outro, e mexer em um não afeta o outro.

O que foi deliberadamente alterado na cópia:

| item | LP Diagnóstico IA | esta LP |
|---|---|---|
| `RD_CONVERSION_IDENTIFIER` | `lp-diagnostico-ia` | `lp-diagnostico-chatgpt` |
| tags no RD Station | `diagnostico-ia`, `vibe-coding` | + `campanha-chatgpt` |
| `og:url` e `canonical` | `lp-diagnostico-ia-lughy.vercel.app` | `lp-diagnostico-chatgpt.vercel.app` |
| porta do servidor local | 8010 | 8020 |

**O identificador de conversão precisa continuar diferente.** Se as duas LPs mandarem
o mesmo, o RD Station junta tudo e não dá para separar o resultado das campanhas, que
é justamente o motivo de existir esta cópia.

Container do GTM é o **mesmo** (`GTM-N384CG7L`), de propósito: a separação acontece
pelo identificador de conversão e pela URL, não por container.

⚠️ **A copy é idêntica à da LP original**, inclusive a lista de ferramentas
("Lovable, Cursor, Replit, bolt.new") e as opções do campo de ferramenta no
formulário — **nenhuma menciona ChatGPT**. Se a campanha for mirar quem construiu com
ChatGPT, isso precisa ser ajustado antes de subir mídia.
...
## Domínio: lp-diagnostico-chatgpt.vercel.app

Fica na URL automática da Vercel, mesmo padrão das LPs irmãs. Sem custo, sem DNS,
deploy automático a cada push. O nome estava livre, então esta sai sem sufixo
(diferente da `lp-diagnostico-ia`, que virou `lp-diagnostico-ia-lughy`).
...
2. Configurar as variáveis de ambiente:
   ...
   - `RD_CONVERSION_IDENTIFIER` — identificador da conversão. Padrão: `lp-diagnostico-chatgpt`.
